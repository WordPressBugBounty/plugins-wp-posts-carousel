...
$template .= '<article class="cci-wpc-slide cci-wpc-free-template cci-wpc-free-template-simple cci-wpc-slides-' . esc_attr( $this->params['all_items'] ) . ' ' . esc_attr( $this->slide_layout_classes() ) . '"' . $this->slide_layout_attributes() . '>';
        if ( $woocommerce_card ) {
    ...
    $template .= '<div class="cci-wpc-card cci-wpc-card-post cci-wpc-container">';
        $template .= do_action( 'wp_posts_carousel_before_item_content', $this->params );
             ...
             * show category
             */
            if ( $this->params['show_category'] === 'true' && ( $this->categories || $eyebrow || $kicker || $story_step ) ) {
                $template .= '<p class="cci-wpc-card-taxonomy cci-wpc-categories">';
                    if ( $eyebrow ) {
            ...
            $template .= '<div class="cci-wpc-card-body cci-wpc-details">';
                ...
                if ( $this->params['show_created_date'] === 'true' || $this->params['show_author'] === 'true' ) {
                    $template .= '<div class="cci-wpc-card-meta cci-wpc-post-details">';
                        ...
                        if ( $this->params['show_created_date'] === 'true' && $this->params['show_author'] === 'true' ) {
                            $template .= '<span class="cci-wpc-post-details-separator">,</span>';
                        }
                ...
                if ( $this->params['show_tags'] === 'true' ) {
                    $template .= '<p class="cci-wpc-card-tags cci-wpc-tags">';
