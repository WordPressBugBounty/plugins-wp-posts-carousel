  ...
  public static function generate($atts)
  {
    ...
    while ($loop->have_posts()) {
      ...
      if ($taxonomies = get_object_taxonomies($post_type, 'objects')) {
        foreach ($taxonomies as $taxonomy) {
          if (preg_match('/category/', $taxonomy->name) || preg_match('/\b_cat\b/i', $taxonomy->name) || preg_match('/\b-cat\b/i', $taxonomy->name)) {
            $post_type_category = esc_attr($taxonomy->name);
          } else if (preg_match('/tag/', $taxonomy->name)) {
            $post_type_tag = esc_attr($taxonomy->name);
          }
        }
      }
      ...
      /*
            ...
            */
      if (!$image || !array_key_exists(0, $image) && $image[0] == '' || $image[0] == '/') {
      ...
      if ($params['show_featured_image'] === 'true') {
        ...
        $data_src = 'src="' . esc_attr($image[0]) . '"';
        $image_class = null;

        if ($params['lazy_load'] === 'true') {
          $data_src = 'data-src="' . esc_attr($image[0]) . '" data-src-retina="' . esc_attr($image[0]) . '"';
          $image_class = 'class="owl-lazy"';
        }

        $featured_image = '<div class="wp-posts-carousel-image">';
        $featured_image .= '<a href="' . esc_attr($post_url) . '" title="' . __('Read more', 'wp-posts-carousel') . ' ' . esc_html($post->post_title) . '">';
        $featured_image .= '<img alt="' . esc_attr($post->post_title) . '" style="max-width:' . esc_attr($params['image_width']) . '%;max-height:' . esc_attr($params['image_height']) . '%" ' . $data_src . $image_class . '>';
        $featured_image .= '</a>';
        $featured_image .= '</div>';
      }
      ...
      if ($params['show_title'] === 'true') {
        ...
        $title .= '<a href="' . esc_attr($post_url) . '" title="' . esc_attr($post->post_title) . '">' . esc_html($post->post_title) . '</a>';
        $title .= '</h3>';
      }
      ...
      if ($params['show_category'] === 'true') {
        ...
        if ($categories_list) {
          ...
          foreach ($categories_list as $cat) {
            $category .= '<a href="' . get_category_link($cat->term_id) . '" title="' . esc_attr(sprintf(__("View all items in %s"), $cat->name)) . '">' . esc_html($cat->name) . '</a> ';
          }
          $category .= '</p>';
        }
      }
      ...
      if ($params['show_description'] === 'excerpt') {
        ...
        $description = '<div class="wp-posts-carousel-desc">' . (esc_html($params['allow_shortcodes']) === 'true' ? do_shortcode(get_the_content('', true)) : get_the_content()) . '</div>';
      }
      ...
      if ($params['show_more_button'] === 'true') {
        ...
        $buttons .= '<a href="' . esc_attr($post_url) . '" class="wp-posts-carousel-more-button button" title="' . __('Read more', 'wp-posts-carousel') . ' ' . esc_html($post->post_title) . '">' . __('read more', 'wp-posts-carousel') . '</a>';
        $buttons .= '<p>';
      }
